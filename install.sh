...
APPDIR="${BUILD_ROOT}/SkyOrcaMax.AppDir"
OUTPUT_DIR="${PROJECT_ROOT}/dist"
APPIMAGE_NAME="SkyOrcaMax-Simulator-1.0.0-x86_64.AppImage"
APPIMAGE="${OUTPUT_DIR}/${APPIMAGE_NAME}"

case "$(uname -m)" in
  x86_64|amd64) APPIMAGE_ARCH="x86_64" ;;
  aarch64|arm64) APPIMAGE_ARCH="aarch64"; APPIMAGE_NAME="SkyOrcaMax-Simulator-1.0.0-aarch64.AppImage"; APPIMAGE="${OUTPUT_DIR}/${APPIMAGE_NAME}" ;;
...
mkdir -p "${APPDIR}/usr/lib/skyorcamax-simulator" "${APPDIR}/usr/share/applications" "${APPDIR}/usr/share/icons/hicolor/scalable/apps"
cp -a "${rm_build_path}/dist/skyorcamax-simulator/." "${APPDIR}/usr/lib/skyorcamax-simulator/"
cp "${PROJECT_ROOT}/packaging/AppRun" "${APPDIR}/AppRun"
cp "${PROJECT_ROOT}/packaging/skyorcamax.desktop" "${APPDIR}/skyorcamax.desktop"
cp "${PROJECT_ROOT}/packaging/skyorcamax.desktop" "${APPDIR}/usr/share/applications/skyorcamax.desktop"
cp "${PROJECT_ROOT}/packaging/skyorcamax.svg" "${APPDIR}/skyorcamax.svg"
cp "${PROJECT_ROOT}/packaging/skyorcamax.svg" "${APPDIR}/usr/share/icons/hicolor/scalable/apps/skyorcamax.svg"
chmod +x "${APPDIR}/AppRun" "${APPDIR}/usr/lib/skyorcamax-simulator/skyorcamax-simulator"
...
INSTALLED_APPIMAGE="${INSTALL_DIR}/SkyOrcaMax-Simulator.AppImage"
cp "${APPIMAGE}" "${INSTALLED_APPIMAGE}"
cp "${PROJECT_ROOT}/packaging/skyorcamax.svg" "${ICON_DIR}/skyorcamax.svg"
...
mkdir -p "${DESKTOP_DIR}"
SHORTCUT="${DESKTOP_DIR}/SkyOrcaMax Simulator.desktop"
sed \
  -e "s|^Exec=.*|Exec=${INSTALLED_APPIMAGE}|" \
  -e "s|^Icon=.*|Icon=${ICON_DIR}/skyorcamax.svg|" \
  "${PROJECT_ROOT}/packaging/skyorcamax.desktop" > "${SHORTCUT}"
chmod +x "${SHORTCUT}"
